...
- 图谱交互：支持拖动画布、缩放、单节点拖动、项目节点框选、组拖动、空白点击取消选择；项目节点点击有 `pointerup` 兜底，避免被拖动/框选逻辑吞掉；拖动后的节点位置和视图缩放会持久化保存。
...
当前节点位置调整会自动保存到本地 `graph/layout.json`。刷新页面、关闭服务后再打开，节点位置和视图缩放/平移会按上次布局恢复。`graph/layout.json` 默认写入 `.gitignore`，避免每次拖动图谱都让 Git 工作区变脏。

## 布局持久化

图谱页面会通过本地接口保存展示状态：

- `GET /api/graph-layout`：读取上次保存的节点坐标和视图状态。
- `POST /api/graph-layout`：保存当前节点坐标、缩放比例和画布平移。

自动保存触发时机包括：拖动单个节点、拖动项目节点框选出的节点组、拖动画布、滚轮缩放和点击“重置视图”。
...
仓库内置 `mcp/llm_wiki_mcp.py`，用于让 Codex 在聊天中直接把本次项目复盘、部署问题、命令片段或技术总结沉淀进知识库。

典型用法：

```text
把这次 RK3588 的部署问题沉淀到我的知识库
```

Codex 可以先整理当前对话和文件改动，再调用 MCP：

```text
llm_wiki.ingest_project_notes
```

MCP 会复用当前 `kb_server.py` 的智能导入逻辑：

```text
Codex 整理 Markdown
  -> 写入 raw/imports
  -> 智能导入 / 项目资料包导入
  -> 拆成 项目 / 技术 / 问题 / 部署 / 命令 节点
  -> 生成候选关系
  -> 重建 graph.json
  -> 返回导入报告
```
